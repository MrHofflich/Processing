...
void setup() {
  ...
  println(Serial.list());
  serialPort = new Serial(this, "/dev/ttyACM1", 9600);
  
  logPracticaAlejandro = loadTable("logPracticaAlejandro.csv", "header");
  IronMan = new arcR(100,110,110);
  objetos= new objeto[1000];
  timer = new Timer(800);
}
...
//serialPort.write(sentData);
  ...
  if(sentData == 'X') {
    ...
    IronMan.altura = (int)map(altura, 0, 255,210,810-IronMan.r/2);
    serialPort.write(sentData);
  }
  */
}
...
//for debugging
  ...
  if(keyPressed == true) {
    ...
    float distance = dist (x, altura, o.x,o.y); //calcula la distancia
    if (distance < x+r + o.r) {  //comparala con la suma de los radios
      return true;
...
class objeto {
  ...
  objeto () {  //Constructor of drops
    ...
    x = width + r*4; //empieza un poco mas afuera de la pantalla
    y = random(height);   //empieza randommente en el eje Y
